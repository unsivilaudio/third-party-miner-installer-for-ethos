...
function start_miner()
{
	...
	********************************/
	...
	if ($miner == "xmrig-nvidia") {
	    delete_old_api_port();
	    $apiport = select_api_port();
	    $devices = implode(",",select_gpus());
	    if(trim(`/opt/ethos/sbin/ethos-readconf selectedgpus`) != ""){
	        $mine_with = "--cuda-devices $devices";
	    }
	...
	$miner_path['xmrig-nvidia'] = "/usr/bin/screen -c /opt/ethos/etc/screenrc.xmrig-nvidia -l -L -dmS xmrig-nvidia /opt/miners/xmrig-nvidia/xmrig-nvidia";
    ...
    if(intval(trim(file_get_contents("/opt/ethos/etc/allow.file"))) == 0) {
		...
		$miner_params['xmrig-nvidia'] = $flags ." ". $pools;
		...
		$miner_suffix['xmrig-nvidia'] = " ". $mine_with ." ". $extraflags;
